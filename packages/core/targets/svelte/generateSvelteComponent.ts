  ...
  const bindings = bindingsConfig
    ...
    .filter(
      (c1, index, self) =>
        index ===
        self.findIndex(
          c2 => c1.event === c2.event && c1.targetProp === c2.targetProp,
        ),
    )
  ...
  .map(
    prop =>
      `export let ${prop.name}${
        !prop.required
          ? ` = ${
              safeDefaultValue(prop.defaultValue)
                ? prop.defaultValue
                : 'undefined'
            }`
          : ''
...
${methods
  .map(
    method =>
      `export const ${method.name} = (...args) => __ref.${method.name}(...args);`,
  )
  .join('\n')}
...
onMount(() => {
  ${properties
    ...
    .map(
      prop =>
        `${prop.name} = (${prop.name} === void 0) ? __ref['${prop.name}'] : ${prop.name};`,
    )
    .join('\n  ')}
...
${properties
  ...
  .map(
    prop => `$: if (__mounted) setProp(__ref, '${prop.name}', ${prop.name});`,
  )
  .join('\n')}
