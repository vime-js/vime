...
import { jsxEventName } from '../targetHelpers';
...
export const replaceSvelteMethodDefs = (meta: ComponentCompilerMeta, source: string) => {
  const name = dashToPascalCase(meta.tagName);

  let newSource = source;

  newSource = source.replace('get ref() {', `get ref(): HTML${name}Element | undefined {`);
