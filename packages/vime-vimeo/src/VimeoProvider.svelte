<VimeoLite
  ...
  on:paramschange={onReload}
  on:paramschange={onRebuildStart}
  on:data={onData}
  bind:this={lite}
/>
...
<script>
  ...
  import VimeoLite from './VimeoLite.svelte'
  ...
  const send = (command, args) => lite && lite.sendCommand(command, args)
  ...
  onMount(() => dispatch(PlayerEvent.ORIGIN_CHANGE, lite.getOrigin()))
