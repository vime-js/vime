...
const getDesc = (event: JsonDocsEvent) => `${event.deprecation !== undefined ? `<span style="color:red">**[DEPRECATED]**</span> ${event.deprecation}<br/><br/>` : ''}${event.docs}`;
...
export const eventsToMarkdown = (events: JsonDocsEvent[]) => {
  ...
  events.forEach((ev) => {
    table.addRow([`\`${ev.event}\``, getDesc(ev), `\`CustomEvent<${ev.detail}>\``]);
  });
