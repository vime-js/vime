...
const languagesOrder = ['html', 'react', 'vue', 'vue 2', 'vue 3', 'svelte', 'stencil', 'angular'];

const formatLanguageLabel = (lang: string) => ((lang === 'html')
  ? lang.toUpperCase()
  : dashToPascalCase(lang));
...
defaultValue="html"
values={[
  ${languages.map((lang) => `{ label: '${formatLanguageLabel(lang)}', value: '${lang}' }`).join(',\n  ')}
]}>`);
