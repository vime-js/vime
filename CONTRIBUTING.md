...
2. Add documentation and usage example to the auto-generated `readme.md` file in the provider's
   directory (above the `<-- Auto Generated Below -->` comment).
...
1. Run the build script `npm run build`.
2. Make sure documentation and a usage example is added to the auto-generated `readme.md` file
   in the component's directory (add it above the `<-- Auto Generated Below -->` comment).
3. When creating a new component, add it to [`VimeModule`](./packages/angular/src/vime-module.ts)
